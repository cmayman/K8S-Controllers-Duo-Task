            ...
            steps {
                script { 
                    if (env.GIT_BRANCH == 'origin/development') {
                        sh '''
                        docker build -f ./Dockerfile -t gcr.io/lbg-python-cohort-8/maymanchris-flask-app:latest .
                        '''
        ...
        stage('Push') {
             steps {
                script { 
                    if (env.GIT_BRANCH == 'origin/development') {
                        sh '''
                        docker push gcr.io/lbg-python-cohort-8/maymanchris-flask-app:latest
                        '''
